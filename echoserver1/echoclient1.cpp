...
{
	public:
		echo_session(asio::io_service& io_service) 
			: socket_(io_service)
		{
			id_ = id;
			++id;
		...
		void start(const std::string& ip, const std::string& port)
		{
			...
			//异步连接
			socket_.async_connect(*iterator, bind(&echo_session::handle_connect, this, std::placeholders::_1));
		}
	private:
		void handle_connect(const error_code& error)
		{
			if (!error)
			{
				...
				asio::async_write(socket_, 
						...
						bind(&echo_session::handle_write, this,
							std::placeholders::_1));
			}
			else
				cout << error << endl;
		...
		void handle_write(const error_code& error)
		{
			...
			{
				//写入完毕，接收服务器回射的消息
				asio::async_read(socket_, asio::buffer(buf_, sizeof(buf_)),
						bind(&echo_session::handle_read, this,
