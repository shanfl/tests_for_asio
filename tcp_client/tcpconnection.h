...
/*, private boost::noncopyable*/
{
  ...
  protected:
    ...
    virtual void onConnectFailure(const asio::error_code &e)
    {
        (void)e;
        //assert(false);
		fprintf(stderr, "onConnectFailure e = %d msg = %s\n", e.value(),e.message().c_str());
    }
    virtual void onReceiveHeadSuccess(DataBuffer &data)
    {
        (void)data;
		//fprintf(stderr, "onReceiveHeadSuccess e = %d\n", e.value());
    }
    ...
    {
        (void)e;
		fprintf(stderr, "onReceiveFailure e = %d,msg = %s\n", e.value(),e.message());
    }
    virtual void onSendSuccess() 
	{ 
		assert(false); 
		fprintf(stderr, "onSendSuccess \n");
	}
    virtual void onSendFailure(const asio::error_code &e)
    {
        (void)e;
		fprintf(stderr, "onSendFailure e = %d\n", e.value());
    }
    virtual void onTimeoutFailure(const asio::error_code &e)
    {
        (void)e;
		fprintf(stderr, "onTimeoutFailure e = %d,msg = %s\n", e.value(),e.message());
    }
    virtual void onCommonError(uint32_t ec, const string &em)
    {
        (void)ec;
        (void)em;
		fprintf(stderr, "onCommonError msg = %s", em.c_str());
    }
